...
_feetech_sdk_available = importlib.util.find_spec("scservo_sdk") is not None


def require_package(pkg_name: str, extra: str = "", import_name: str | None = None) -> None:
    name = import_name or pkg_name
    if not importlib.util.find_spec(name):
        raise ImportError(f"'{pkg_name}' is required but not installed. Run: pip install {pkg_name}")
from .tables import (
